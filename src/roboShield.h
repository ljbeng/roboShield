...
/*  LJB  4/17/18 modifed Library for use on roboShield
This is the original PCA9685 with the TB6612 Library thrown in.  The PCA chip is broken out
so PWM outputs 0-7 are available for the programmers use.  outputs 9-15 are used to control
the PWM and the Digital pins to the TB6612FNG motor driver chip.  setChannelPWM was modified
so no PWM channel over channel 8 can be controlled by that call.  Only calls to the motor functions
will alter the state of PWM 9 - 15.
...
PWM7 - USER PWM 
PWM8 - UNUSED 
PWM9 - PWM FOR MOTOR B 
PWM10 - BIN2 FOR MOTOR B (DIGITAL PIN, DRIVE ONLY TO FULL ON OR FULL OFF) 
PWM11 - BIN1 FOR MOTOR B (DIGITAL PIN, DRIVE ONLY TO FULL ON OR FULL OFF) 
PWM12 - STDBY FOR TB6612 (DIGITAL PIN, DRIVE ONLY TO FULL ON OR FULL OFF) 
PWM13 - AIN1 FOR MOTOR A (DIGITAL PIN, DRIVE ONLY TO FULL ON OR FULL OFF) 
PWM14 - AIN2 FOR MOTOR A (DIGITAL PIN, DRIVE ONLY TO FULL ON OR FULL OFF) 
PWM15 - PWM FOR MOTOR A 

All the features of the TB6612 library are included here
.brake
.drive
.standby
.forward
...
public:
    ...
    roboShield(TwoWire& i2cWire = Wire, PCA9685_PhaseBalancer phaseBalancer = PCA9685_PhaseBalancer_Linear, int enMotor = 1);
#else
    ...
    roboShield(PCA9685_PhaseBalancer phaseBalancer = PCA9685_PhaseBalancer_Linear, int enMotor = 1);
#endif
...
class roboShield_ServoEvaluator {
public:
    ...
    roboShield_ServoEvaluator(uint16_t n90PWMAmount = 102, uint16_t p90PWMAmount = 512);
    ...
    roboShield_ServoEvaluator(uint16_t n90PWMAmount, uint16_t zeroPWMAmount, uint16_t p90PWMAmount);

    ~roboShield_ServoEvaluator();
